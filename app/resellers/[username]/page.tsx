...
import { normalizeExternalUrl } from '@/lib/validators/linkValidators';
...
export default function ResellerProfilePage() {
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col font-mono text-xs text-text-muted justify-center items-center gap-4">
        ...
        <p className="uppercase tracking-widest text-white">Loading Sentinel Reseller Profile...</p>
      </div>
    );
  }
  ...
  const backupWhatsappNumber = reseller.backup_whatsapp_number || reseller.backupWhatsappNumber;
  ...
  return (
    <ProtectedRoute
      ...
      description="Authentication is strictly required to view Sentinel Verified store profiles, direct contact numbers, peer endorsements, and escrow partnerships."
    >
              ...
              {whatsappNumber ? (
                <div className="p-5 rounded-2xl border border-emerald-500/30 bg-emerald-950/20 backdrop-blur-md space-y-3 shadow-[0_0_30px_rgba(16,185,129,0.05)]">
                  ...
                  <div className="space-y-2 pt-1">
                    ...
                    {whatsappGroupLink && (
                      <a
                        href={normalizeExternalUrl(whatsappGroupLink)}
                        target="_blank"
                    ...
                    {backupWhatsappNumber && (
                      <div className="pt-2.5 mt-2 border-t border-emerald-500/15 flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <span className="text-[10px] text-emerald-400/80 uppercase font-mono block">Backup WhatsApp Line</span>
                          <span className="text-xs font-mono text-white font-bold truncate block">{backupWhatsappNumber}</span>
                        </div>
                        <a
                          href={`https://wa.me/${backupWhatsappNumber.replace(/[^0-9]/g, '')}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="px-2.5 py-1 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 border border-emerald-500/30 text-[11px] font-mono flex items-center gap-1 transition-all shrink-0 cursor-pointer"
                        >
              ...
              {telegramUsername ? (
                <div className="p-5 rounded-2xl border border-sky-500/30 bg-sky-950/20 backdrop-blur-md space-y-3 shadow-[0_0_30px_rgba(14,165,233,0.05)]">
                  ...
                  <div className="space-y-2 pt-1">
                    ...
                    {telegramChannelLink && (
                      <a
                        href={normalizeExternalUrl(telegramChannelLink)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-full py-2 px-3 rounded-xl bg-white/5 hover:bg-white/10 text-white border border-white/10 text-xs font-medium flex items-center justify-center gap-2 transition-all truncate"
                      >
